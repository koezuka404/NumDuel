...
type GameDeps struct {
	...
	Secrets      domain.SecretHasher
	Locks        domain.GameLockStore
...
func buildGameState(ctx context.Context, d GameDeps, userID, gameID uuid.UUID) (*GameStateOutput, error) {
	...
	if err != nil {
		return nil, domain.ErrInternal("failed to find game")
	}
	if game == nil {
		return nil, domain.ErrNotFound("game not found")
	}
	if !game.IsParticipant(userID) {
		return nil, domain.ErrForbidden("not a participant")
	}
	myGuesses, err := d.Repo.Guesses().ListByGameAndPlayer(ctx, gameID, userID)
	if err != nil {
		return nil, domain.ErrInternal("failed to load guesses")
	}
	...
	if err != nil {
		return nil, domain.ErrInternal("failed to count opponent guesses")
	}
	remaining := 0
	if d.Turns != nil && game.Status == domain.GameStatusInProgress {
		remaining, err = d.Turns.RemainingSeconds(ctx, gameID, d.now())
		if err != nil {
			return nil, domain.ErrInternal("failed to read turn deadline")
		}
	}
	turnPlayer := ""
	...
	for i, g := range myGuesses {
		summaries[i] = GuessSummary{
			Turn: g.Turn, GuessNumber: g.GuessNumber,
			DigitResults: domain.DigitResultsToInts(g.DigitResults),
...
func SetSecretNumber(ctx context.Context, d GameDeps, userID, gameID uuid.UUID, secret string) error {
	if d.Locks != nil {
		ok, err := d.Locks.AcquireLock(ctx, infrredis.SecretLockKey(gameID, userID), d.GameLockTTL)
		if err != nil {
			return domain.ErrInternal("failed to acquire secret lock")
		}
		if !ok {
			return domain.ErrRateLimitExceeded()
		}
	}
	secretNum, err := domain.NewSecretNumberFromString(secret)
	if err != nil {
		return err
	}
	...
	if err := withTx(ctx, d.Repo, func(tx domain.Transaction) error {
		...
		if err != nil {
			return domain.ErrInternal("failed to find game")
		}
		if game == nil {
			return domain.ErrNotFound("game not found")
		}
		if !game.IsParticipant(userID) {
			return domain.ErrForbidden("not a participant")
		}
		switch game.Status {
		case domain.GameStatusWaitingSecret:
		case domain.GameStatusInProgress:
			return domain.ErrGameAlreadyStarted()
		case domain.GameStatusFinished:
			return domain.ErrGameAlreadyFinished()
		default:
			return domain.ErrValidation("invalid game status")
		}
		...
		if err != nil {
			return domain.ErrInternal("failed to hash secret")
		}
		...
		if err := d.Repo.Games().Update(ctx, tx, game); err != nil {
			return domain.ErrInternal("failed to save secret")
		}
...
func startGameInTx(ctx context.Context, d GameDeps, tx domain.Transaction, game *domain.Game) error {
	now := d.now()
...
func notifyGameStart(ctx context.Context, d GameDeps, gameID uuid.UUID) error {
	game, err := d.Repo.Games().FindByID(ctx, gameID)
	if err != nil || game == nil {
		return domain.ErrInternal("failed to load started game")
	}
	now := d.now()
	...
	if d.Turns != nil && game.CurrentTurnPlayerID != nil {
		if err := d.Turns.SetTurn(ctx, gameID, game.CurrentTurn, *game.CurrentTurnPlayerID, now, expires); err != nil {
			return domain.ErrInternal("failed to set turn deadline")
		}
	}
...
func notifyTurnChanged(ctx context.Context, d GameDeps, game *domain.Game) error {
	...
	if d.Turns != nil {
		...
		if err != nil {
			return domain.ErrInternal("failed to read turn deadline")
		}
	}
...
func SubmitGuess(ctx context.Context, d GameDeps, userID, gameID uuid.UUID, guess string, isAuto bool) error {
	if d.Locks != nil && !isAuto {
		ok, err := d.Locks.AcquireLock(ctx, infrredis.GuessLockKey(gameID, userID), d.GameLockTTL)
		if err != nil {
			return domain.ErrInternal("failed to acquire guess lock")
		}
		if !ok {
			return domain.ErrRateLimitExceeded()
		}
	}
	guessNum, err := domain.NewGuessNumberFromString(guess)
	if err != nil {
		return err
	}
	if err := ensureGamePlayer(ctx, d.Repo, userID); err != nil {
	...
	if err := withTx(ctx, d.Repo, func(tx domain.Transaction) error {
		...
		if opponentHash == "" {
			return domain.ErrValidation("opponent secret not registered")
		}
		...
		if err := d.Repo.Guesses().Create(ctx, tx, &g); err != nil {
			return domain.ErrInternal("failed to save guess")
		}
		isWin := domain.IsWin(results)
		...
		if isWin {
			...
			return domain.ErrInternal("failed to update game turn")
		}
		if game.CurrentTurnPlayerID != nil {
...
func finishGameService(ctx context.Context, repo domain.Repository, tx domain.Transaction, game *domain.Game, winnerID uuid.UUID, now time.Time) error {
	if winnerID == uuid.Nil {
		return domain.ErrInternal("winner is required")
	}
	...
	if err != nil || winner == nil {
		return domain.ErrInternal("failed to find winner")
	}
	loser, err := repo.Users().FindByID(ctx, loserID)
	if err != nil || loser == nil {
		return domain.ErrInternal("failed to find loser")
	}
	if err := game.Finish(winnerID, now); err != nil {
		return err
	}
	if err := repo.Games().Update(ctx, tx, game); err != nil {
		return domain.ErrInternal("failed to finish game")
	}
	history := domain.NewMatchHistory(game.ID, winnerID, loserID, winner.Username, loser.Username, now)
	if err := repo.MatchHistories().Create(ctx, tx, &history); err != nil {
		return domain.ErrInternal("failed to create match history")
	}
	if err := repo.Users().IncrementWinCount(ctx, tx, winnerID, now); err != nil {
		return domain.ErrInternal("failed to increment win count")
	}
	return nil
}
...
func notifyGuessResult(ctx context.Context, d GameDeps, gameID uuid.UUID, result guessResultOutput, finished bool, winnerID uuid.UUID) error {
	game, err := d.Repo.Games().FindByID(ctx, gameID)
	if err != nil || game == nil {
		return domain.ErrInternal("failed to load game")
	}
	...
	if d.Turns != nil && game.CurrentTurnPlayerID != nil {
		...
		if err := d.Turns.SetTurn(ctx, gameID, game.CurrentTurn, *game.CurrentTurnPlayerID, now, now.Add(d.TurnDuration)); err != nil {
			return domain.ErrInternal("failed to set next turn")
		}
	}
	return notifyTurnChanged(ctx, d, game)
}

func ensureGamePlayer(ctx context.Context, repo domain.Repository, userID uuid.UUID) error {
	...
	if user.IsMaster() {
		return domain.ErrForbidden("master cannot play")
	}
	return nil
}
