...
func MatchPlayers(ctx context.Context, d MatchingDeps, tx domain.Transaction) (*domain.Game, error) {
	entries, err := d.Repo.MatchingQueue().FindWaitingForUpdate(ctx, tx, 2)
	if err != nil {
		return nil, domain.ErrInternal("failed to load matching queue")
	}
	if len(entries) < 2 {
		return nil, nil
	}
	...
	if p1.UserID == p2.UserID {
		return nil, domain.ErrInternal("invalid matching pair")
	}
	if ok, err := matchingPlayerReady(ctx, d.Repo, p1.UserID); err != nil {
		return nil, domain.ErrInternal("failed to validate matching player")
	} else if !ok {
	...
	now := d.now()
	game := &domain.Game{
		ID: uuid.New(), Status: domain.GameStatusWaitingSecret,
	...
	if err := d.Repo.Games().Create(ctx, tx, game); err != nil {
		return nil, domain.ErrInternal("failed to create game")
	}
...
func StartMatching(ctx context.Context, d MatchingDeps, userID uuid.UUID) (*StartMatchingOutput, error) {
	...
	if user.IsMaster() {
		return nil, domain.ErrForbidden("master cannot start matching")
	}
	active, err := d.Repo.Games().FindActiveByUserID(ctx, userID)
	if err != nil {
		return nil, domain.ErrInternal("failed to check active game")
	}
	if active != nil {
		return nil, domain.ErrUserInActiveGame()
	}
	waiting, err := d.Repo.MatchingQueue().ExistsWaitingByUserID(ctx, userID)
	if err != nil {
		return nil, domain.ErrInternal("failed to check matching queue")
	}
	if waiting {
		return nil, domain.ErrAlreadyInMatching()
	}
	var matched *domain.Game
	now := d.now()
	if err := withTx(ctx, d.Repo, func(tx domain.Transaction) error {
		if err := d.Repo.MatchingQueue().Insert(ctx, tx, &domain.MatchingQueueEntry{
			ID: uuid.New(), UserID: userID, Status: domain.MatchingQueueWaiting, CreatedAt: now,
		}); err != nil {
			return domain.ErrInternal("failed to join matching queue")
		}
...
func matchingPlayerReady(ctx context.Context, repo domain.Repository, userID uuid.UUID) (bool, error) {
	...
	active, err := repo.Games().FindActiveByUserID(ctx, userID)
	if err != nil {
		return false, err
	}
...
func removeQueueEntries(ctx context.Context, repo domain.Repository, tx domain.Transaction, ids []uuid.UUID) error {
	if err := repo.MatchingQueue().DeleteByIDs(ctx, tx, ids); err != nil {
		return domain.ErrInternal("failed to cleanup matching queue")
	}
	return nil
}
