...
func (h *Handler) Handle(c echo.Context) error {
	...
	for {
		...
		if userID == uuid.Nil {
			if msgType != "AUTH" {
				h.writeError(conn, domain.CodeUnauthorized, "authentication required")
				continue
			}
			...
			now := time.Now().UTC()
			_ = h.Repo.WSConnectionLogs().Create(ctx, &domain.WSConnectionLog{
		...
		case "SET_SECRET":
			...
			if err != nil {
				h.Hub.SendError(userID, domain.CodeValidation, "invalid gameId")
				continue
			}
		...
		default:
			h.Hub.SendError(userID, domain.CodeValidation, "unknown event type")
		}
	}
}
...
func (h *Handler) onDisconnect(ctx context.Context, userID uuid.UUID) {
	...
	active, err := h.Repo.Games().FindActiveByUserID(ctx, userID)
...
func (h *Handler) sendDomainError(userID uuid.UUID, err error) {
	...
	h.Hub.SendError(userID, domain.CodeInternalError, "internal server error")
}
...
func (h *Handler) writeDomainError(conn *gorillaws.Conn, err error) {
	...
	_ = h.writeJSON(conn, map[string]any{
		"type": "ERROR", "data": map[string]string{"code": domain.CodeInternalError, "message": "internal server error"},
	})
}
