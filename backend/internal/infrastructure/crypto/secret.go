...
var _ domain.SecretHasher = (*SecretHashService)(nil)
...
func (s *SecretHashService) Verify(storedHash string, guess domain.GuessNumber, gameID uuid.UUID, opponentSlot int) ([4]domain.DigitResult, error) {
	parts := strings.Split(storedHash, ":")
	if len(parts) != 4 {
		return [4]domain.DigitResult{}, domain.ErrInternal("invalid secret hash format")
	}
	digits := guess.Digits()
	var results [4]domain.DigitResult
	for i := 0; i < 4; i++ {
		expected := s.digest(gameID, opponentSlot, i, digits[i])
		if hmac.Equal([]byte(parts[i]), []byte(expected)) {
			results[i] = domain.DigitHit
		}
	}
	return results, nil
}
