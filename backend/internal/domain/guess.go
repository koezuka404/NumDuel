...
// SecretNumber は秘密の 4 桁数字（値オブジェクト, 仕様書 4.1, 1.1）。
//
// ルール:
//   - 4 桁固定
//   - 各桁 0〜9
...
// Entity / DB に平文を保持しない（仕様書 3.10.2）。
type SecretNumber struct {
	digits [4]int // 非公開。Digits() / String() 経由でのみ参照
}

// NewSecretNumber は [4]int から SecretNumber を生成（仕様書 4.1 NewSecretNumber）。
...
// Guess は予想履歴 Entity（仕様書 9.6 guesses テーブル）。
//
// 追加は必ず Game.AddGuess 経由（仕様書 4.3）。
...
type Guess struct {
	...
	CreatedAt    time.Time
	UpdatedAt    time.Time     // バックアップ差分 UPSERT 用（仕様書 9.14）
}
