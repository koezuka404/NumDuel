...
type SecretNumber struct {
	digits [4]int // 非公開Digits() / String() 経由でのみ参照
}
...
// Digits は内部 4 桁配列を返すSecretHashService.Hash の入力に UseCase が使用
func (s SecretNumber) Digits() [4]int {
	return s.digits
}

// String は 4 桁文字列化ログ出力禁止
func (s SecretNumber) String() string {
	return digitsToString(s.digits)
}

// GuessNumber は予想 4 桁数字（値オブジェクト）
// SecretNumber と同一の桁ルールSubmitGuess / HandleTimeout の入力検証に使う
...
// guess_number は自分の予想のみ平文保存相手の予想内容は WS に含めない
type Guess struct {
